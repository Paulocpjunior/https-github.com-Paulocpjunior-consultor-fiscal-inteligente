import { analisarLotePisCofins } from './pisCofins.js';
import { analisarLoteIcms }      from './icms.js';
...
export async function analisarCreditos(notas, perfilCliente) {
  if (!notas || notas.length === 0) throw new Error('Nenhuma nota fiscal informada para análise');
  if (!perfilCliente || !perfilCliente.regime) throw new Error('Perfil do cliente incompleto — regime tributário obrigatório');
  ...
  const totaisConsolidados = {
    ...
    resumo: { pisCofins: pisCofins.totais, icms: icms.totais },
  };
...
function gerarAlertas(pisCofins, icms, perfilCliente) {
  const alertas = [];
  if (pisCofins.totais.totalRevisar > 0) alertas.push({ nivel: 'ATENCAO', mensagem: `${pisCofins.totais.totalRevisar} nota(s) com PIS/COFINS a revisar manualmente` });
  if (icms.totais.totalRevisar > 0)      alertas.push({ nivel: 'ATENCAO', mensagem: `${icms.totais.totalRevisar} nota(s) com ICMS a revisar manualmente` });
  if (icms.totais.totalParcial > 0)      alertas.push({ nivel: 'INFO',    mensagem: `${icms.totais.totalParcial} nota(s) com ativo imobilizado — controlar CIAP` });
  if (perfilCliente.regime === 'LUCRO_PRESUMIDO') alertas.push({ nivel: 'INFO', mensagem: 'Regime cumulativo — PIS/COFINS não geram crédito (Lucro Presumido)' });
  return alertas;
}
