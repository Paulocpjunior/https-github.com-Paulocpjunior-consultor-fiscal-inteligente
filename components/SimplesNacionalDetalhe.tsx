...
const SimplesNacionalDetalhe: React.FC<SimplesNacionalDetalheProps> = ({ 
    ...
    const handleSaveMesVigente = async () => {
        ...
        try {
            ...
            // Critical Fix: Append to existing history safely
            ...
            // Await the update to ensure it persists before showing success
    ...
    const handleSaveHistorico = async () => {
        if (window.confirm('Deseja salvar o faturamento manual? Isso substituirá os dados existentes.')) {
            // Must wrap in async handling to ensure toast shows AFTER DB write
            try {
    ...
    const handleGerarDasPdf = async () => { 
        ...
        try {
            ...
            // --- NOVA PÁGINA: HISTÓRICO DE APURAÇÕES ---
            if (filteredHistory && filteredHistory.length > 0) {
                ...
                doc.setTextColor(255, 255, 255);
                doc.setFontSize(16);
                doc.text("Histórico de Apurações Salvas", 105, 12, { align: 'center' });
                
                doc.setFontSize(8);
                doc.text(`Gerado em: ${dataHora}`, 14, 20);
                doc.text(`Empresa: ${empresa.nome}`, 196, 20, { align: 'right' });
                ...
                doc.setFont(undefined, 'bold');
                doc.text("Registro Completo", 14, yHist);
                yHist += 6;

                // Tabela Header
                doc.setFillColor(240, 240, 240);
                doc.rect(14, yHist, 182, 8, 'F');
                doc.setFontSize(9);
                doc.text("Competência", 16, yHist + 5);
                doc.text("RBT12 (R$)", 90, yHist + 5, { align: 'right' });
                doc.text("Aliq. Efetiva", 140, yHist + 5, { align: 'right' });
                doc.text("Valor DAS (R$)", 190, yHist + 5, { align: 'right' });
                yHist += 12;
                ...
                filteredHistory.forEach((hist, index) => {
                    ...
                    if (yHist > pageHeight - 20) {
                        doc.addPage();
                        yHist = 20;
                        // Se necessário, repetir o header aqui
                    }

                    if (index % 2 === 0) {
                        doc.setFillColor(250, 250, 250);
                        doc.rect(14, yHist - 4, 182, 7, 'F');
                    }

                    // Formata string de competência para garantir primeira letra maiúscula
                    const mesFormatado = hist.mesReferencia.charAt(0).toUpperCase() + hist.mesReferencia.slice(1);

                    doc.text(mesFormatado, 16, yHist);
                    doc.text(hist.rbt12.toLocaleString('pt-BR', { minimumFractionDigits: 2 }), 90, yHist, { align: 'right' });
                    doc.text(`${hist.aliq_eff.toFixed(2)}%`, 140, yHist, { align: 'right' });
                    doc.text(hist.das_mensal.toLocaleString('pt-BR', { minimumFractionDigits: 2 }), 190, yHist, { align: 'right' });
                    
                    yHist += 7;
                });

                // Rodapé da Página de Histórico
                doc.setFontSize(8);
                doc.setTextColor(150);
                doc.text(footerText, 14, pageHeight - 10);
                doc.text("SP Assessoria Contábil", 196, pageHeight - 10, { align: 'right' });
